...
7. 添加host虚拟主机支持，同一个ip地址的多个虚拟主机通过hostname进行区分
8. 添加管道化机制。之前服务器在发送返回体后需要等待下一个请求，管道化后，请求可以同时发起，不过因为请求没有序号，所以服务器需要接收顺序按序返回，没有解决队头阻塞；浏览器厂商使用多tcp连接实行并发
...
## 5. http3
0. http2的缺陷：tcp队头阻塞(tcp滑动窗口卡住)、tcp握手延迟
...
## 2.2 HEAD方法作用
在请求大文件的时候使用，返回资源在GET请求下返回头的信息，但是不需要返回资源本体，可以在下载资源前获取资源的信息，如content-length等
...
2. 多了很多选项
#### 优先级
cache-control比expires更加靠谱，所以同时存在的话，会优先使用cache-control

## 2. 协商缓存
在强缓存过期后，浏览器会发送请求到服务器询问资源是否发生了变化，可以复用则返回304，不可以复用则返回200和新资源
### 1. last-modified
#### 使用头部
1. last-modifiled 服务器返回资源上次被修改时间
2. if-modified-since 浏览器发送上次last-modifiled时间，服务器比较资源修改时间是否一致
#### 缺点
1. 无法判断一秒内多次修改
2. 资源内容没有发生变化但重写
...
#### 使用头部
1. etag 服务器返回资源标志
2. if-match 浏览器发送上次etag结果，对比资源etag是否一致
#### 优势
1. 可以识别一秒内多次修改的情况
2. 资源内容没有修改不会返回
#### 劣势
...
# http2头部压缩原理
- 使用了字典来进行压缩，双方都会维护两份字典，一份是静态字典，包括常见的头部名称或者值，所有人都是同一份静态字典；一份是动态字典，可以动态新增。
- 第一次传输的时候，发送者可以使用静态字典上的索引来表示对应的首部信息，比如1表示`method:get`，如果不在静态字典上的，可以做上标记，第一次传输后接收者就可以把首部信息存储到动态表上，下一次就可以使用对应的索引来代替了。
...
所以说，两个不同域的会握两次手，同域的请求会都在同一个连接上发送
...
## TCP和HTTP的区别是什么
TCP是传输层协议为了保证上层数据的准确性，HTTP是应用层协议，双方用于理解对方意图的规范
